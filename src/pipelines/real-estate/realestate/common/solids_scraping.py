...
#     """
#     The purpose of the list_objects method is to define metadata for each partition that is ingested
#     by this ReadConnector. The established keys in the return value are associated, unique
#     identification values for each partition. Specifically, the 'fingerprint' of a partition will
#     help determine whether or not that partition has changed. If a 'fingerprint' for a partition
#     changes, then the partition will be reprocessed. If the 'fingerprint' for a partition does not
#     change, no reprocessing will occur.

#     # Returns
#     Iterator(Dictionary): Each dictionary has three key values
...
#         'is_prefix' - A boolean that represents whether or not the current partition holds any child
#                       partitions
#     """
#     # db_cursor = context['db_connection'].cursor()

#     try:
#         # return all property-ids per citiy
#         plist = list_props_immo24(IN_PROPERTY_TYPE, IN_RENT_OR_BUY, IN_RADIUS, IN_CITIES)

#     except Exception as e:
#         raise Exception(
#             'Error getting a list of properties from list_immoScraper with error message: {}'.format(
#                 e
#             )
#         )
#     try:
...
#             yield l
#     except Exception as e:
#         raise Exception('Error yielding property in list_objects-method: {}'.format(p, e))
...
#     # Returns
#     Dictionary: You should be returning useful information/objects that can be used in the following
#                 two methods: list_objects and read_bytes. This return value is passed into each of
#                 those two methods as the 'context' parameter.
#                 e.g. You can return SDK Client objects, DB cursors, configuration parameters, etc.
#     """
#     pass
...
#     """
#     The read_bytes method performs the grunt operations of a ReadConnector. This is where the raw
#     data for a ReadConnector is processed and scanned.

#     # Returns
#     A byte stream that represents all data to return for a given ReadConnector configuration
#     """
#     props = []
#     try:

#         # if price has changes for already downloaded item
#         if metadata['fingerprint']:
#             id = metadata['name']
#             prop = fetch_property_from_api_to_json(id)

#             # add newest changed price to dataset
#             today = datetime.today().strftime('%Y-%m-%d')
#             # TODO: retrieve normalized price before

#             # # access properties from ascend:
#             # A = Client(
#             #     'trial.ascend.io',
#             #     os.environ['ASCEND_ACCESS_KEY_ID'],
#             #     os.environ['ASCEND_SECRET_ACCESS_KEY'],
#             # )
#             # data = A.get_component(
#             #     os.environ['ASCEND_ACCOUNT'],
#             #     'Immoscout24',
#             #     'scrape_new_properties_from_ImmoScout24_API',
#             # )

#             prop['propertyDetails']['normalizedPrice_' + today] = prop['propertyDetails'][
#                 'normalizedPrice'
#             ]
...
#     except Exception as e:
#         raise Exception(
#             'Error scraping propertyIds from ImmoScout24-API with PropertyId {} and with the error message: {}'.format(
#                 metadata['name'], e
#             )
#         )
#     try:
...
#     except Exception as e:
#         raise Exception('Error yielding property in read_bytes-method: {}'.format(p, e))


# # def run_immoScraper(propertyType, rentOrBuys, radius, cities, path):
# #     scrape_immo24(propertyType, rentOrBuys, radius, cities, path)
# #     return 'scraping finished'


# # run_immoScraper(IN_PROPERTY_TYPE, IN_RENT_OR_BUY, IN_RADIUS, IN_CITIES, IN_PATH)
